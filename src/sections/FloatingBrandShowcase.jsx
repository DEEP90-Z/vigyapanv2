...
export default function FloatingBrandShowcase() {
  // Smart dynamic bubble generation algorithm:
  // Guarantees zero adjacent duplicate logos, uniform vertical staggering, and non-colliding horizontal lanes.
  const bubbles = useMemo(() => {
    ...
    const sways = [16, -22, 25, -18, 20, -24, 18, -20, 22, -16];

    return Array.from({ length: TOTAL_BUBBLES }).map((_, index) => {
      // 1. Round-robin logo assignment (guarantees different logos in sequence)
      ...
      // 2. Calculated duration & mathematical negative delay staggering
      const duration = 24 + (index % 4) * 2; // 24s, 26s, 28s, 30s
  ...
  return (
    <section className="relative min-h-[900px] sm:min-h-[1050px] w-full flex items-center justify-center overflow-hidden bg-[#FAF9F5] select-none py-24 sm:py-36">
      {/* Keyframe for Smooth Infinite Upward Floating */}
      <style>{`
      ...
      {/* Ambient Luxury Glow Orbs (Gold, Rose & Purple Accents) */}
      <div className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
        ...
        {/* Decorative Ambient Concentric Rings */}
      ...
      {/* Centered Luxury Impact Content (z-10) */}
      <div className="relative z-10 w-full max-w-3xl text-center flex flex-col items-center justify-center my-auto px-4 pointer-events-auto">
        ...
        >
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2.5 px-4.5 py-2 rounded-full bg-white/90 backdrop-blur-xl border border-purple-900/15 text-xs font-semibold uppercase tracking-widest text-purple-950 mb-7 shadow-[0_4px_20px_rgba(147,51,234,0.08)] cursor-pointer transition-all hover:border-purple-400 hover:shadow-[0_6px_25px_rgba(147,51,234,0.15)]"
          >
          ...
          <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-3.5">
            {IMPACT_HIGHLIGHTS.map((chip, index) => (
              <motion.span
                key={chip}
                ...
                className="px-3.5 py-1.5 rounded-full bg-white/70 backdrop-blur-md border border-stone-300/60 text-[11px] sm:text-xs font-semibold text-stone-700 shadow-sm transition-all hover:bg-white hover:border-purple-300 hover:text-purple-950"
              >
                {chip}
              </motion.span>
            ))}
      ...
      {/* Floating Logo Bubbles Layer (z-30: Positioned OVER central text) */}
